<!-- Generated from keepalived.conf.5 by tools/man2md.py. Do not edit by hand. -->
...
The include variants add additional include checks to the current include_check level (see below) The variants are:  
...
**&lt;TIMER&gt;** is a time value in seconds, including fractional seconds, e.g. 2.71828 or 3; resolution of timer is micro-seconds.
...
For specifying scripts with parameters, unquoted spaces will separate the parameters. If it is required for a parameter to contain a space, it should be enclosed in single quotes ('). For example
...
global_defs {
    ...
    # This is to get around the problem of there having been two simultaneous
    ...
    #   If priority is 255 ignore received adverts
    vrrp_strict
    ...
    # keepalived starts. See also vrrp_delay_after_boot below.
    vrrp_startup_delay 5.5

    # Probably a better alternative to vrrp_startup_delay  is to use
    # vrrp_delay_after_boot. This will ensure that vrrp instances are delayed starting
    # until at least the specified number of seconds after the system boots. Therefore,
    # when keepalived is started at boot time, it can delay starting the VRRP instances
    # until the specified time has elapsed, but if keepalived is stopped and restarted
    # it will not have any delay. keepalived will use the longer delay of
    # vrrp_startup_delay and vrrp_delay_after_boot.
    ...
    # of the file to be written. Note that data_use_instance may also modify the
    # file name.
    ...
    # iproute can use two directories for its configuration files, with files in
    # /etc/iproute2 overriding files in /usr/share/iproute2.
    # ip (the package that provides ip route functionality) has configure options
    # for the location of these directories. keepalived's configure will attempt
    # to find the locations either from the ip-route man page or the ip utility
    # itself. In case it cannot do so, the locations may need to be specified here.
...
If the global vrrp_garp_interval and/or vrrp_gna_interval are set, any interfaces that aren't specified in a garp_group will inherit the global settings on a per interface basis.
...
vrrp_instance <STRING> {
    ...
    # For compliance with RFC9568, this option should be set.
    v3_checksum_as_v2 [<BOOL>]
    ...
    # 5.5 seconds (if not disabled by "nopreempt").
...
The delays are specified in seconds, with a resolution of microseconds, e.g. a delay of 0.00001 means 10 usecs. A delay of 0 means no delay in state change. The maximum delay that can be specified is 255 seconds.

If up_delay is omitted, it is set to be the same as the down delay.

The down delay on an interface must be less than two (or more precisely one less than down_timer_adverts (default 3)) times the advert interval of any VRRP instance using that interface (otherwise a backup instance, while not receiving adverts may time out and become master before this instance transitions to FAULT state). Consequently the up/down delays can be dynamically reduced if another instance is master with a shorter advert interval.

If the VRRP instance is using a VMAC, it will inherit the up/down debounce delays of its parent interface.

## LVS CONFIGURATION

contains subblocks of **Virtual server group(s)** and **Virtual server(s)**

The subblocks contain arguments for configuring Linux IPVS (LVS) feature. Knowledge of

*ipvsadm(8)*

will be helpful here. Configuring LVS is achieved by defining virtual server groups, virtual servers and optionally SSL configuration. Every virtual server defines a set of real servers, you can attach healthcheckers to each real server. Keepalived will then lead LVS operation by dynamically maintaining topology.
...
**${_RANDOM [MIN [MAX]]} :** This is replaced by a random integer in the range [MIN, MAX], where MIN and MAX are optional non-negative integers. Defaults are MIN=0 and MAX=32767.  
**${_ENV ENV_VAR} :** This is replaced by the value of the environment variable $ENV_VAR.  
**${_HASH} :**  This is replaced by a '#' character, which would otherwise start a comment  
**${_BANG} :**  This is replaced by a '!' character, which would otherwise start a comment

...
During the configure step of the compiling process, you will get a configuration summary before building with `make`. For example, you may see similar output on a CentOS machine:

    ./configure --prefix=/usr/local/keepalived-2.3.4 --enable-snmp
    Keepalived configuration
    ------------------------
    Keepalived version       : 2.3.4
    Compiler                 : gcc
...
You can query keepalived SNMP managed objects by using the OID. For example:
    ...
    SNMPv2-SMI::enterprises.9586.100.5.1.1.0 = STRING: "Keepalived v2.3.4"
...
Alternatively, with the keepalived MIB, you can query using the MIB available from the project. First, copy the MIB to the system's global MIB directory or to the user's local MIB directory:
    ...
    cp /usr/local/src/keepalived-2.3.4/doc/KEEPALIVED-MIB /usr/share/snmp/mibs

or:

    cp /usr/local/src/keepalived-2.3.4/doc/KEEPALIVED-MIB ~/.snmp/mibs
...
The SNMP daemon will check both directories for the existence of the MIB. Once the MIB is in place, the SNMP query can look as follows:
    ...
    KEEPALIVED-MIB::version.0 = STRING: Keepalived v2.3.4

...
The most common keepalived command line arguments are listed below. For the
authoritative and complete list of options for your version, run
`keepalived --help` or consult the `keepalived(8)` man page.
...
On a systemd based distribution, start Keepalived with:

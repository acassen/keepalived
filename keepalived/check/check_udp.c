...
static void
free_udp_check(__attribute__((unused)) checker_t *checker)
{
}
...
static bool
udp_check_compare(const checker_t *a, const checker_t *b)
{
...
{
	check_update_svr_checker_state(is_success, THREAD_ARG(thread), thread, "UDP", udp_connect_thread);
}

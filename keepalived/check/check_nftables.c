...
process_fwmark_vsge_range(const struct sockaddr_storage *addr, const struct sockaddr_storage *addr_end, uint16_t service_type, unsigned fwmark, int cmd)
{
	...
	do {
		...
		if (addr->ss_family == AF_INET) {
			...
			if (!inet_sockaddrcmp(PTR_CAST(struct sockaddr_storage, &sockaddr6), addr_end))
				break;
...
{
	unsigned fwmark = vs->vsg->auto_fwmark ? vs->vsg->auto_fwmark : get_next_fwmark();
...
{
	do_vs_fwmark(vs, vs->vsg->auto_fwmark, NFT_MSG_DELSETELEM);
}

void
remove_vs_fwmark_entry(virtual_server_t *vs, virtual_server_group_entry_t *vsge)
{
	process_fwmark_vsge_range(&vsge->addr, &vsge->addr_end, vs->service_type, vs->vsg->auto_fwmark, NFT_MSG_DELSETELEM);
}

...
static void
add_del_rules(int cmd, bool ignore_errors)
{
	struct iptc_handle *h4;
	struct ip6tc_handle *h6;
...
static int
iptables_entry(struct ipt_handle* h, const char* chain_name, unsigned int rulenum, const char* target_name, const ip_address_t* src_ip_address, const ip_address_t* dst_ip_address, const char* in_iface, const char* out_iface, uint16_t protocol, uint8_t type, int cmd, bool force)
{
	int res;
...
static void
handle_iptable_rule_to_NA_lib(ip_address_t *ipaddress, int cmd, void *h, bool force)
{
...
handle_iptable_rule_to_vip_lib(ip_address_t *ipaddress, int cmd, struct ipt_handle *h, bool force)
{
	...
	if (IP_IS6(ipaddress)) {
		if (IN6_IS_ADDR_LINKLOCAL(&ipaddress->u.sin6_addr))
			ifname = ipaddress->ifp->ifname;
		handle_iptable_rule_to_NA_lib(ipaddress, cmd, h, force);
	}
...
void
iptables_startup_lib(bool reload)
{

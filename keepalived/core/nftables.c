...
log_message(LOG_INFO, "mnl_socket_recvfrom returned %d", ret);
		ret = mnl_cb_run(buf, ret, 0, portid, NULL, NULL);
log_message(LOG_INFO, "mnl_cb_run returned %d", ret);
		if (ret <= 0)
			break;
	}
log_message(LOG_INFO, "After mnl_recv_loop, ret = %d, errno %d", ret, errno);
	FREE(buf);
